...
dirname = "Single_sims/Final_sims/const_pl_var_alpha/"
...
for i in alp_vals:
	...
	all_bets = np.zeros ((iterations,4,len(periods)))
	xips = np.zeros ((iterations,4,len(periods)))
	xims = np.zeros ((iterations,4,len(periods)))
	for j in range(1,iterations+1):
		all_bets[j-1] = np.load(dirname+'pl_{0}_alp_sim_{1}_{2}_{3}.npy'.format(round(100*i),round (K*10),round(gp*100),j))[0]
		xips[j-1] = np.load(dirname+'pl_{0}_alp_sim_{1}_{2}_{3}.npy'.format(round(100*i),round (K*10),round(gp*100),j))[1]
		xims[j-1] = np.load(dirname+'pl_{0}_alp_sim_{1}_{2}_{3}.npy'.format(round(100*i),round (K*10),round(gp*100),j))[2]
...
left, bottom, width, height = [.65, 0.20, 0.25, 0.25]
...
p1, = ax.plot(alp_vals, xipm_avg, "b-", label=r"$\langle \xi_+^2 + \xi_-^2 \rangle$")
p2, = twin1.plot(alp_vals,alp_vals*4, "r-", label="MPCR")
...
ax_new.errorbar (alp_vals, A_avg,yerr=var_avg)
...
plt.errorbar (alp_vals, A_avg,yerr = var_avg)
plt.xlabel ('Group size $N$')
plt.ylabel (r'$\langle A \rangle$')
plt.show()

plt.plot (alp_vals,var_avg)
plt.show()
